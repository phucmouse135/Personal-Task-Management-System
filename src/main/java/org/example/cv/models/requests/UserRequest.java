...
public class UserRequest {
    ...
    @Size(max = 50, message = "Username must not exceed 50 characters", groups = {OnCreate.class, OnUpdate.class})
    @Schema(description = "Unique username of the user", example = "john_doe", maxLength = 50, required = true)
    String username;

    @NotBlank(message = "Password is required", groups = {OnCreate.class})
    @Size(max = 100, message = "Password must not exceed 100 characters", groups = {OnCreate.class})
    @Schema(description = "Password of the user", example = "securePassword123", maxLength = 100, required = true)
    String password;

    @Size(max = 100, message = "First name must not exceed 100 characters", groups = {OnCreate.class, OnUpdate.class})
    @Schema(description = "First name of the user", example = "John", maxLength = 100)
    String firstName;

    @Size(max = 100, message = "Last name must not exceed 100 characters", groups = {OnCreate.class, OnUpdate.class})
